...
[HarmonyPatch(typeof(GenText), nameof(GenText.IsValidFilename))]
public static class GenText_IsValidFilename
{
    private static bool Prefix(ref bool __result, string str)
    {
        __result = str.Length <= 60 &&
                   !new Regex($"[{Regex.Escape(GenText.GetInvalidFilenameCharacters())}]").IsMatch(str);
